...
const Profile = ({ userId }) => {
    ...
    const defaultProfileImage = 'https://team-ssc.s3.ap-northeast-2.amazonaws.com/%EA%B8%B0%EB%B3%B8+%ED%94%84%EB%A1%9C%ED%95%84+%EC%82%AC%EC%A7%84.webp';
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            try {
                const response = await axios.get(`http://localhost:8080/api/users/7/profile`); // 나중에 {userId} 바꿔야함
            ...
            {isModalOpen && (
                <Modal onClose={closeModal}>
                    <div className={styles.modalContent}>
                        <button className={styles.modalButton} onClick={() => navigate(`/profile/edit/image/${userId}`)}>
                            프로필 이미지 수정
                        </button>
                        <button className={styles.modalButton} onClick={() => navigate(`/profile/edit/details/${userId}`)}>
                            프로필 내용 수정
                        </button>
                    </div>
                </Modal>
